# PLAN-MKP-002: MouseKeyProxy - Free Hotkey-Only Alternative to PowerToys Mouse Without Borders

**Status**: Superseded (tombstoned per Round 4 review). Forked into PLAN-MKP-003.md (and then 004). All content moved forward. Do not edit.
...
| TLS server private key (mTLS) | Service (data plane) | Service account (LocalSystem or configured) | %ProgramData%\MouseKeyProxy\certs\ (ACL: service read, admin write only) | Machine store or LocalMachine + entropy | Service unit + install ACL test |
...
- PerMonitorV2 required for Agent (TR-MKP-INPUT-001 covers input fidelity; add explicit if new TR needed in store).
...
Merge (LIFO): push or move-to-top on dup. Max 50 entries (or ~1MB total size cap, whichever first).
...
The block below is the locked, intended-to-be-compilable contract. Validation: run `protoc --proto_path=. --csharp_out=. mousekeyproxy.proto` (or buf lint) as part of plan verification gates.
...
service MouseKeyProxy {
  // Primary cross-machine channel: remote dials host (bidi stream carries all high-rate + control)
  rpc OpenSession (OpenSessionRequest) returns (stream SessionEnvelope);

  // Pairing protocol surface (for discovery/key negotiation before full session)
  rpc Pair (PairRequest) returns (PairResponse);

  // Local REPL/tray helpers (routed over stream in practice)
...
message SessionEnvelope {
  oneof payload {
    InputBatch input = 1;
    ClipboardPush clip = 2;
    ControlMsg control = 3;
    Heartbeat hb = 4;
    Ack ack = 5;
  }
}
...
(End of locked proto contract. Full simple messages like SetMouse* are identical to prior version. Compile with protoc as gate.)

**Validation evidence**: Successful `protoc --proto_path=src/Common/Network --csharp_out=gen mousekeyproxy.proto` (or buf lint) required before network Elaboration. Add to verification gates.
```

**Validation evidence**: The plan requires a successful `protoc` or `buf build` dry-run before any Elaboration code for the network layer. See verification gates.
