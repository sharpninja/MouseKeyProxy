...
MouseKeyProxy supports a physical Raspberry Pi Zero 2 HID appliance path for lab validation and future backend work. The appliance is implemented in C# on .NET 10 and is published self-contained for `linux-arm`.

For **Orange Pi Zero 2** (Allwinner H616 / Armbian / `linux-arm64`), see `docs/hardware/orange-pi-zero-2-hid.md` and `scripts/pi/prepare-orange-pi-zero-2-sd.ps1`. Do not flash Raspberry Pi OS onto an Orange Pi board.
...
- Distribution: Raspberry Pi OS Lite, 32-bit ARM (armhf), based on Debian 11 "bullseye".
- Kernel: supplied by the `2023-02-21` Raspberry Pi OS Bullseye armhf image.
- Default image: `2023-02-21-raspios-bullseye-armhf-lite.img.xz`.
...
- The image ships no .NET runtime; the HID service is published self-contained for `linux-arm` and deployed separately (below).
...
```powershell
dotnet publish src/MouseKeyProxy.PiHid/MouseKeyProxy.PiHid.csproj -c Release -r linux-arm --self-contained true
```
...
1. Install Raspberry Pi OS Lite 32-bit ARM (Debian 11 bullseye, armhf) and set hostname `mkp-hid-pi`. `mkp pi provision` does this and the SSH/Wi-Fi/user setup for you.
2. Copy the published `linux-arm` output to `/opt/mousekeyproxy/pi-hid`.
