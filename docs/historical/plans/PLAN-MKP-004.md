...
**Date**: 2026-07-02 / 2026-07-03 (Round 4 combined 2026-07-03)
...
- Stale "pwsh 5.1" fixed to "powershell.exe (5.1)".
- Unused OpenSessionRequest removed from actual .proto file; re-checked with protoc -- PASSED.
- Review history and status updated. All per the exact list in this Codex review.
All claims in summary now match body; store and plan aligned.
...
Wireframes (01-tray-icon-menu.svg etc.) are Inception/Elaboration deliverable that block tray ACs and tests (see docs/wireframes/ to be populated; specs in original PLAN-MKP-001). Post-completion successor plan ID: docs/PLAN-MKP-004-Release.md (or PLAN-MKP-REL-001).
...
- README.md (extensive)
- LICENSE (MIT)
- docs/PLAN-MKP-004.md (this working plan) + guides + wireframes/
...
- `mkp service install` (requires elevation): extracts/copies service payload (from tool package or side-by-side), `sc create` "MouseKeyProxy", adds firewall rules (via elevated Windows PowerShell 5.1 `powershell.exe`), starts and validates.
